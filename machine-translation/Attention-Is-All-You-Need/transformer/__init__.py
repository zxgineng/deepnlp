...
class Graph:
    ...
    def build(self,
        ...
        # embedding and add(inputs,position)
        # shape:[batch_size,max_seq_length,model_dim]
        encoder_emb_inp = self.build_embed(encoder_inputs, encoder=True)
        # send encoder into the network
        # shape:[batch_size,max_seq_length,model_dim]
        ...
        # when predicting, use last decoder output as next decoder input
        else:
            next_decoder_inputs = self._filled_next_token(decoder_inputs, output, 1)

            # use loop to predict next output
            def condition(i,next_decoder_inputs,output):
                cond = tf.less(i,Config.data.max_seq_length)
                return cond

            def body(i,next_decoder_inputs,output):
                ...
                return [i+1, next_decoder_inputs,next_output]

            i = 2
            [i,next_decoder_inputs,next_output] = tf.while_loop(condition,body,[i,next_decoder_inputs,output])
    ...
    def build_embed(self, inputs, encoder=True, reuse=False):
        """embeding one-hot into dense matrix"""
        with tf.variable_scope("Embeddings", reuse=reuse, dtype=self.dtype) as scope:
            ...
            position_inputs = tf.reshape(position_inputs,
                                         [self.batch_size, Config.data.max_seq_length])

            if encoder:
                embedding_inputs = embedding_encoder
            else:
                embedding_inputs = embedding_decoder
            # embedding word and position, then add
            # shape:[batch_size,max_seq_length,model_dim]
    ...
    def build_output(self, decoder_outputs, reuse=False):
        with tf.variable_scope("Output", reuse=reuse):
            # shape[batch_size,max_seq_length,target_vocab_size]
            logits = slim.fully_connected(decoder_outputs,Config.data.target_vocab_size,activation_fn=None)
    ...
    def _filled_next_token(self, inputs, logits, decoder_index):
        """
        slice the last decode output as next decoder input.
        :param inputs: 2D tensor, shape:[batch_size,max_seq_length], original decoder input before embedding
        :param logits: 3D tensor, shape:[batch_size,max_seq_length,vocab_size]
        :param decoder_index: int,
        """
