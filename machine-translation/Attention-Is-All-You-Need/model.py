...
class Model:
    ...
    def _init_placeholder(self, features, labels):
        # pad token and concat start_token
        self.encoder_inputs = features["enc_inputs"]
        self.targets = labels
        self.batch_size = tf.shape(self.encoder_inputs)[0]

        start_tokens = tf.fill([self.batch_size, 1], Config.data.START_ID)
        # input ground truth with start of start_token when training
        if self.mode == tf.estimator.ModeKeys.TRAIN:
            # slice last pad token of target and concat start_token
        ...
        # input only start_token when eval
        else:
    ...
    def _build_optimizer(self):
        ...
        float_step = tf.to_float(global_step)
        learning_rate = tf.multiply(Config.model.model_dim ** -0.5, tf.reduce_min(
            [tf.pow(float_step, -0.5), float_step * (Config.train.warmup_steps ** -1.5)]), name='learning_rate')
