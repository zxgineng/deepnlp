...
def get_dataset_batch(data, buffer_size=10000, batch_size=64, scope="train"):
    """create input func and initial hook"""
    ...
    def inputs():
        with tf.name_scope(scope):
            ...
            # Create tensor for LoggingTensorHook
...
def build_vocab(in_fname, out_fname):
    """
    Count the vocab of the train enc and train dec.
    Write into the files
    :param in_fname: filename of train.enc
    :param out_fname: filename of train.dec
    """
    print("Count each vocab frequency ...")

    def count_vocab(fname):
        """Convert vocab frequency into a dictionary"""
        vocab = {}
...
def token2id(data, mode):
    ...
    for line in tqdm(lines):
        ...
        # add end and pad index at the end of decoder
        if mode == 'dec':
            ids.append(vocab['<\s>'])
...
def process_data():
    ...
    # count the vocab and write into the files
    build_vocab('train.enc', 'train.dec')
    # convert all the train & test encoder & decoder into corresponding index
    # write into the files
    token2id('train', 'enc')
    token2id('train', 'dec')
    token2id('test', 'enc')
...
def load_data(enc_fname, dec_fname, train=True):
    ...
    for e_line, d_line in tqdm(zip(enc_input_data.readlines(), dec_input_data.readlines())):
        ...
        if len(e_ids) == 0 or len(d_ids) == 0:
            continue
        # pad the seq until max seq length
