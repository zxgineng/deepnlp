...
class PRFScoreHook(tf.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        return tf.train.SessionRunArgs(["next_batch:4", "next_batch:3", 'prediction:0'])

    def after_run(self, run_context, run_values):
        labels, length, preds = run_values.results
        for i in range(len(labels)):
            la = data_loader.id2label(labels[i], self.label_dict)
            le = length[i]
            pr = data_loader.id2label(preds[i], self.label_dict)
